...
### 0.1.7 — fix the Xcode Cloud package-resolution failure (2026-09-14)

Every Xcode Cloud build since the morning of 2026-09-14 failed at the archive's
dependency-resolution step, before any Swift compiled: "a resolved file is
required when automatic dependency resolution is disabled … Package.resolved …
dependencies were added: 'fluidaudio'". The `.xcodeproj` and its workspace are
generated by XcodeGen at build time and never committed, so no `Package.resolved`
exists, and the Xcode Cloud workflow has automatic resolution turned off.
`ci_scripts/ci_post_clone.sh` now runs `xcodebuild -resolvePackageDependencies`
right after `xcodegen generate`, which writes `Package.resolved` to the exact
path Xcode Cloud expects. Alternative one-click fix, if this ever regresses:
turn on automatic dependency resolution in the Xcode Cloud workflow settings.
