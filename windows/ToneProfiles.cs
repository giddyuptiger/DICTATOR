...
/// <summary>
/// The register the speaker picks by hand. Mirrors DictationMode in
/// Sources/DictationCore/ToneProfile.swift. The system prompt sent to the backend
/// is <c>Base + "\n\n" + mode.Instructions</c>.
///
/// (The macOS/iOS apps also mix in a per-app "tone profile" inferred from the
/// frontmost app's bundle id. Windows v1 does not do per-app inference — it just
/// uses the hand-picked mode. Per-app profiles are a documented future add.)
/// </summary>
...
{
    /// <summary>
    /// The label previews the mode's own output: "super casual" is lowercase because
    /// that is exactly what that mode produces.
    /// </summary>
    public static string DisplayName(this DictationMode mode) => mode switch
    {
        DictationMode.SuperCasual => "super casual",
        DictationMode.Casual => "Casual",
        DictationMode.Formal => "Formal",
        DictationMode.Expressive => "Expressive",
        DictationMode.Emoji => "Emoji",
        DictationMode.Patois => "Patois",
        DictationMode.Shakespearean => "Shakespearean",
        _ => mode.ToString(),
    };

    /// <summary>
    /// These modes rewrite the speaker's WORDS (a translation), not just punctuation.
    /// The cleanup fidelity guards (which fall back to the raw transcript when too few
    /// original words survive) must be relaxed for them, or the rewrite gets discarded.
    /// See <see cref="Cleaner.Reconcile"/>.
    /// </summary>
    public static bool TransformsWording(this DictationMode mode) =>
        mode is DictationMode.Patois or DictationMode.Shakespearean;

    /// <summary>
    /// The full cleanup system prompt for a mode: the shared base plus the mode's own
    /// instructions, joined the same way the Swift app joins them.
    /// </summary>
    public static string SystemPrompt(this DictationMode mode) =>
        Base + "\n\n" + Instructions(mode);

    public static string Instructions(this DictationMode mode) => mode switch
    {
        DictationMode.SuperCasual => SuperCasualInstructions,
        DictationMode.Casual => CasualInstructions,
        DictationMode.Formal => FormalInstructions,
        DictationMode.Expressive => ExpressiveInstructions,
        DictationMode.Emoji => EmojiInstructions,
        DictationMode.Patois => PatoisInstructions,
        DictationMode.Shakespearean => ShakespeareanInstructions,
        _ => CasualInstructions,
    };

    // NOTE: verbatim strings (@"...") preserve newlines literally; a doubled quote
    // ("") is a single literal quote. The Swift source used '\'-continuations to join
    // wrapped lines into one logical line; those are already joined here.

    // MARK: - Base cleanup prompt (ported verbatim from ToneProfile.swift `base`)
...
The test is whether the speaker was listing or narrating. Narration that merely contains the word ""first"" is still narration. When it is a close call, prose is the safer error: a stray paragraph reads as normal writing, while stray bullets read as a machine got hold of it.";
    ...
    // MARK: - Per-mode instructions (ported verbatim from DictationMode.instructions)

    private const string SuperCasualInstructions =
@"SUPER CASUAL. Write how people actually text. ALWAYS lowercase, including the first letter of every sentence and the word ""i""; only capitalize a proper noun that truly needs it. Contractions everywhere (I'm, gonna, wanna, dont). Drop sentence-initial subjects where speech did (""gonna head out"" not ""I am going to head out""). Minimal punctuation: no semicolons, few commas, and no period on the final line. Never formalize a word the speaker said casually. This must read visibly more casual than normal writing.";
    ...
    private const string CasualInstructions =
@"CASUAL. Normal everyday writing. Sentence case, contractions, ordinary punctuation. Friendly but not sloppy. This is the default register for talking to someone you know.";
    ...
    private const string FormalInstructions =
@"FORMAL. Complete, grammatical sentences. NO contractions at all: expand every one (""I am"" not ""I'm"", ""do not"" not ""don't"", ""cannot"" not ""can't""). Sentence case with precise punctuation. Prefer written connectives over spoken ones. Do not add flourish, length, or business vocabulary the speaker did not use. This must read visibly more formal than normal writing, while keeping the speaker's meaning exactly.";
    ...
    private const string ExpressiveInstructions =
...
A calm or merely factual sentence keeps its period, and do not manufacture excitement that is not there. NEVER change or add words — only punctuation carries the feeling.";
    ...
    private const string EmojiInstructions =
@"CASUAL, and add exactly one emoji — placed at the most expressive spot in the message, which is usually NOT the end. Find the word or moment the emoji plays off best and put it right after that, inline: ""grabbing coffee ☕ before the meeting"", ""that meeting was brutal 😮‍💨"". Only fall back to the very end when the whole message builds to one beat. Exactly one emoji for the whole message; choose it from what that spot is about, not a generic smiley, and never use it to replace a word the speaker said. Always add one.";
    ...
    private const string PatoisInstructions =
...
- Sound natural and warm, not a caricature: if forcing a word would read as mockery, leave that word in plain English. Still drop filler and fix obvious mis-hearings.";
    ...
    private const string ShakespeareanInstructions =
